                ...
                if (val > 0):
                    ...
                    if (val > (2 ** (TotalLen - FracLen) - (2 ** (TotalLen - FracLen - 1)))):
                        if (ReturnVal == 'None'): print("\nERROR: Value is too high, range from",
                                                        ...
                                                        "to",
                                                        -(2 ** (TotalLen - FracLen) - (2 ** (TotalLen - FracLen - 1))),
                                                        " ( value:", val, " index:", Values.index(val), ")")
                        return None
                    elif (val == (2 ** (TotalLen - FracLen) - (2 ** (TotalLen - FracLen - 1)))):
                        if (ReturnVal == 'None'): print("WARNING:",
                                                        (2 ** (TotalLen - FracLen) - (2 ** (TotalLen - FracLen - 1))),
                                                        "can not be represented,", round(
                                ((2 ** (TotalLen - FracLen) - (2 ** (TotalLen - FracLen - 1))) - 1 / (2 ** TotalLen)),
                                Precision), "will be used instead", "( index:", Values.index(val), ")")
                ...
                # If negative
                    ...
                    if ((-1) * val > (2 ** (TotalLen - FracLen) - (2 ** (TotalLen - FracLen - 1)))):
                        if (ReturnVal == 'None'): print("\nERROR: Value is too low, range from",
                                                        ...
                                                        " ( value:", val, " index:", Values.index(val), ")")
                        return None
                    num = (2 ** TotalLen) + (2 ** (TotalLen - FracLen)) + int(
                        val * (2 ** (TotalLen - (TotalLen - FracLen))) - (2 ** (TotalLen - FracLen)))
                    # Check if value is less than minimal possible
                    if (num == 2 ** TotalLen):
                        num = 0
                    hex_text = hex_text + ("0x" + hex(num)[2:].zfill(int(TotalLen / 4))) + ","
        ...
        if (Signed):
            for val in Values:
                ...
                if (val < (2 ** (TotalLen - 1))):
                    ...
                    if (ReturnVal != 'None'): dec_vals.append(
                        round((val / (2 ** (TotalLen - (TotalLen - FracLen)))), Precision))
                else:
                    dec_text = dec_text + Precision_txt.format(
                        val / (2 ** (TotalLen - (TotalLen - FracLen))) - (2 ** (TotalLen - FracLen))) + ","
                    if (ReturnVal != 'None'):  dec_vals.append(
                        round((val / (2 ** (TotalLen - (TotalLen - FracLen))) - (2 ** (TotalLen - FracLen))),
                              Precision))
